...
<nav>
  ...
  <div class="lienprincipals_nav">
    ...
    <div class="liens_nav_containers">
      ...
      <a href="#" aria-current="page">Signalement</a>
    </div>
  </div>
...
$sql = "SELECT utilisateurs.id as user_id, pseudo, message, chat.id as message_id, signalement.id as signalement_id
        FROM utilisateurs
        JOIN chat ON utilisateurs.id = chat.id_envoi
        LEFT JOIN signalement ON chat.id = signalement.id_message";
$result = $conn->query($sql);

// Vérifier s'il y a des résultats
if ($result->num_rows > 0) {
    echo "<table border='1'>";
    echo "<tr><th>Pseudo</th><th>Utilisateur ID</th><th>Message</th><th>Action</th></tr>";
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo "<td>";
        echo "<button onclick='deleteUser(" . $row["user_id"] . ")'>Supprimer Utilisateur</button>";
        echo "<span>&nbsp;&nbsp;</span>"; // Espace entre les boutons
        echo "<button onclick='deleteSignalement(" . $row["signalement_id"] . ")'>Supprimer Signalement</button>";
        echo "</td>";
        echo "</tr>";
    }
...
function deleteUser(userId) {
    // Ajoutez ici le code pour supprimer l'utilisateur en utilisant AJAX ou une autre méthode
    alert("Supprimer utilisateur avec ID: " + userId);
}

// Fonction pour supprimer un signalement
function deleteSignalement(signalementId) {
    // Ajoutez ici le code pour supprimer le signalement en utilisant AJAX ou une autre méthode
    alert("Supprimer signalement avec ID: " + signalementId);
}
</script>
